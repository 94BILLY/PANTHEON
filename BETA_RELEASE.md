...
| **Intro** | Boot luma ramp + `WWW.94BILLY.COM`; libdraw GS coords fixed. **Staggered letter reveal** on by default; **sine wave off** by default (`PANTHEON_BOOT_TEXT_WAVE_AMP=0`) to avoid shimmer — re-enable with e.g. `EE_CFLAGS='-DPANTHEON_BOOT_TEXT_WAVE_AMP=3.5f'`. |
