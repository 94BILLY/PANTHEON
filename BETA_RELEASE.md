...
| Area | Behavior |
|------|----------|
| **Render** | **Hybrid** (`PANTHEON_RENDER_PROFILE=0`): Path1 draws **floor then skydome** (dome after ground avoids dome “eating” the lower screen on some Software-GS paths). **CPU GIF floor is skipped when Path1 floor runs** — avoids Z-fight. |
