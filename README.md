...
<p align="center"><sub><strong>In progress:</strong> final polish on IOP audio (<code>audsrv</code>) — intro theme playback and streaming path. Builds link <code>-laudsrv</code>; see root <code>theme_intro.wav</code> and embedded <code>theme_wav_data.h</code>.</sub></p>
...
- **IOP audio (beta)** — `LIBSD` + `AUDSRV` load, `audsrv` format + streaming hook for **theme intro** (embedded PCM in `theme_wav_data.h`; companion `theme.ads` / `theme_ads_data.h` for ADPCM-style data). **Final tuning in progress** (volume, chunk cadence, hardware vs PCSX2 parity).
- **Reference media** — `theme_intro.wav` at repo root and under `docs/media/` (same bytes; raw GitHub URL friendly for tooling)
...
| `floor.c` | EE conductor — DMA sequencing, boot, world loop, **audsrv** theme init/stream |
| `hrc2ps2.py` | Softimage 3D `.hrc` → C array mesh cruncher |
| `theme_wav_data.h` | Embedded **theme_intro** PCM for `audsrv` (generated / baked asset) |
| `theme.ads` / `theme_ads_data.h` | ADPCM-style theme data companion to WAV path |
| `theme_intro.wav` | Lossless reference WAV (root + `docs/media/`) |
...
- **Phase 2 — Audio + texturing** — Ship reliable **audsrv** theme playback on hardware and PCSX2; then STQ, texture sampling in VU1, GS VRAM upload
