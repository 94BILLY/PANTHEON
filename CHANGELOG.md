...
- **`docs/pantheon-landing.html`** — WordPress Custom HTML aligned with live 94billy.com block: logo, responsive type, Path 1 pipeline section title, wallpaper strip; **Phase 1** bullets for DualShock, hybrid walkable floor, IOP **audsrv** theme audio (beta); install notes for **audsrv** / `-laudsrv`; strict build uses **`EE_EXTRA_CFLAGS`**.
...
- **`docs/pantheon-landing.html`:** Dropped the centered **in progress** audio banner; WAV link lives on the **IOP Theme Audio** bullet only.
