...
VECTOR colours[4] = {
    {0.15f,0.50f,0.10f,1.0f},{0.20f,0.55f,0.08f,1.0f},
    {0.12f,0.45f,0.12f,1.0f},{0.18f,0.52f,0.09f,1.0f},
};
...
// BUILDING - tall box, reused for all 4 buildings
...
VECTOR camera_position = { 0.00f, 10.00f, 80.00f, 1.00f };
VECTOR camera_rotation = { 0.15f,  0.00f,  0.00f, 1.00f };
...
// Draw one building at world pos (wx,wz) relative to player
void draw_building(int b, float px, float pz, float yaw,
    ...
    MATRIX lw, ls;
    int i;
    // Rotate building position around player by -yaw
    ...
    *qp = draw_prim_start(*qp, 0, prim, color);
    for(i=0;i<36;i++){(*qp)->dw[0]=col[cube_points[i]].rgbaq;(*qp)->dw[1]=xyz[cube_points[i]].xyz;(*qp)++;}
...
int render(framebuffer_t *frame, zbuffer_t *z) {
    ...
    VECTOR  *tmp_f = memalign(128,sizeof(VECTOR)*vertex_count);
    xyz_t   *xyz_f = memalign(128,sizeof(vertex_t)*vertex_count);
    color_t *col_f = memalign(128,sizeof(color_t)*vertex_count);
    VECTOR  *tmp_c = memalign(128,sizeof(VECTOR)*cube_vertex_count);
    xyz_t   *xyz_c = memalign(128,sizeof(vertex_t)*cube_vertex_count);
    color_t *col_c = memalign(128,sizeof(color_t)*cube_vertex_count);
    VECTOR  *tmp_b = memalign(128,sizeof(VECTOR)*24);
    xyz_t   *xyz_b = memalign(128,sizeof(vertex_t)*24);
    color_t *col_b = memalign(128,sizeof(color_t)*24);

    prim_t prim;
    color_t color;
    packet_t *packets[2];
    packets[0]=packet_init(600,PACKET_NORMAL);
    packets[1]=packet_init(600,PACKET_NORMAL);
    ...
    for(;;) {
        ...
        calculate_vertices(tmp_f, vertex_count, vertices, local_screen);
        draw_convert_xyz(xyz_f,2048,2048,32,vertex_count,(vertex_f_t*)tmp_f);
        draw_convert_rgbq(col_f,vertex_count,(vertex_f_t*)tmp_f,(color_f_t*)colours,0x80);
        ...
        for(i=0;i<points_count;i++){q->dw[0]=col_f[points[i]].rgbaq;q->dw[1]=xyz_f[points[i]].xyz;q++;}
        q=draw_prim_end(q,2,DRAW_RGBAQ_REGLIST);

        // Player
        q=draw_prim_start(q,0,&prim,&color);
        for(i=0;i<cube_points_count;i++){q->dw[0]=col_c[cube_points[i]].rgbaq;q->dw[1]=xyz_c[cube_points[i]].xyz;q++;}
        q=draw_prim_end(q,2,DRAW_RGBAQ_REGLIST);
